...
curl --fail --location --progress-bar --output "$tmp/bun.zip" "$bun_uri" || error "Failed to download bun from \"$bun_uri\""

# Verify checksum
echo "${expected_sha}  $tmp/bun.zip" | sha256sum -c - || error "Checksum mismatch for $bun_uri"

# Extract and install
unzip -oqd "$tmp" "$tmp/bun.zip" || error 'Failed to extract bun'

install -m 755 "$tmp/bun-$target/bun" "$install_dir/bun" || error 'Failed to install bun'
